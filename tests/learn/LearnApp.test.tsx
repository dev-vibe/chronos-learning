...
describe('Learn route and interactions', () => {
  ...
  it('opens the published Farming and Settlements lesson', async () => {
    ...
    const map = within(document.querySelector('.historical-map-pair')!).getByRole('img', { name: /Editorial locator map.*Çatalhöyük/i });
    const mapPair = map.closest('.historical-map-pair');
    expect(mapPair?.firstElementChild?.classList.contains('knowledge-block')).toBe(true);
    expect(mapPair?.lastElementChild).toBe(map.closest('figure'));
    expect(map.closest('figure')?.querySelector('figcaption')).toBeNull();
    expect(within(map.closest('figure')!).queryByText('Site coordinate verified · modern coasts for orientation')).toBeNull();
    expect(screen.queryByText('This archive entry is not available.')).toBeNull();
  });
  it('gives the portrait Human Origins map the same intentional pair without trapping its prose in the grid', async () => {
    const gateway = new TestGateway();
    ...
    const map = within(document.querySelector('.historical-map-pair')!).getByRole('img', { name: /Map of Africa with five marked sites/i });
    expect(map.getAttribute('width')).toBe('1600');
    expect(map.getAttribute('height')).toBe('1644');
    const mapPair = map.closest('.historical-map-pair');
    expect(mapPair?.querySelector('.historical-map-intro')).toBeTruthy();
    expect(mapPair?.querySelector('.prose-module')).toBeNull();
    expect(map.closest('[data-section-id]')?.querySelector('.section-modules')?.children).toHaveLength(2);
    expect(map.closest('figure')?.querySelector('figcaption')).toBeNull();
    expect(within(map.closest('figure')!).queryByText('Coordinates verified per site · modern coasts for orientation')).toBeNull();
  });
  ...
  it('persists prompt feedback, completes, centers one card reveal, and gives a truthful next action', async () => { const gateway = new TestGateway(); render(<LearnApp lessonId="lesson.uruk.first-city" gatewayFactory={async () => gateway} />); await screen.findByRole('heading', { name: 'Uruk: Life in an Early City' }); await userEvent.click(screen.getByLabelText('Administrative tablets and cylinder seals')); await userEvent.click(screen.getAllByRole('button', { name: 'Compare your thinking' })[0]); expect(await screen.findByText(/Compare the evidence/)).toBeTruthy(); const text = screen.getByRole('textbox'); await userEvent.type(text, 'Specialized work created opportunity, while unequal labor was a serious cost.'); fireEvent.blur(text); await userEvent.click(screen.getAllByRole('button', { name: 'Compare your thinking' })[1]); await waitFor(() => expect(screen.getByRole('button', { name: 'Complete lesson' }).hasAttribute('disabled')).toBe(false)); await userEvent.click(screen.getByRole('button', { name: 'Complete lesson' })); expect(await screen.findByText('Knowledge Card acquired')).toBeTruthy();
    expect(document.querySelector('.card-class')?.textContent).toContain('Place'); const reveal = document.querySelector('.card-reveal'); await waitFor(() => expect(reveal?.scrollIntoView).toHaveBeenCalledWith({ behavior: 'smooth', block: 'center' })); expect(document.activeElement).toBe(reveal); expect(screen.getAllByRole('heading', { name: 'Uruk' })).toHaveLength(1); const next = screen.getByRole('link', { name: /Continue: From Marks/ }); expect(next.getAttribute('href')).toBe('/learn/lesson.writing.early-systems'); });
