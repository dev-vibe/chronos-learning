...
describe('Learn route and interactions', () => {
  ...
  it('persists prompt feedback, finishes by sending answers for review, and gives a truthful next action', async () => { const gateway = new TestGateway(); render(<LearnApp lessonId="lesson.uruk.first-city" gatewayFactory={async () => gateway} />); await screen.findByRole('heading', { name: 'Uruk: Life in an Early City' }); expect(screen.getByText(/you earn the Uruk Knowledge Card/)).toBeTruthy(); await userEvent.click(screen.getByLabelText('Administrative tablets and cylinder seals')); await userEvent.click(screen.getByRole('button', { name: 'Check my answer' })); expect(await screen.findByText(/best-supported answer/)).toBeTruthy(); const text = screen.getByRole('textbox'); await userEvent.type(text, 'Specialized work created opportunity, while unequal labor was a serious cost.'); fireEvent.blur(text); await userEvent.click(screen.getByRole('button', { name: 'Save my answer' })); await waitFor(() => expect(screen.getByRole('button', { name: 'Finish and send for review' }).hasAttribute('disabled')).toBe(false)); await userEvent.click(screen.getByRole('button', { name: 'Finish and send for review' }));
  ...
  it('requires deliberate comparison and keeps a short draft when reopened', async () => {
    ...
    expect(screen.queryByText(/Specialized workers|An example explanation/)).toBeNull();
