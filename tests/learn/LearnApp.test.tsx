...
describe('Learn route and interactions', () => {
  ...
  it('keeps the journey outline in a drawer and returns focus after Escape', async () => { const gateway = new TestGateway(); render(<LearnApp lessonId="lesson.uruk.first-city" gatewayFactory={async () => gateway} />); await screen.findByRole('heading', { name: 'Uruk: Life in an Early City' }); const rail = screen.getByLabelText('World History World Spine'); expect(rail.getAttribute('aria-hidden')).toBe('true'); const open = screen.getByRole('button', { name: 'World Spine', exact: true }); await userEvent.click(open); expect(screen.getByRole('dialog', { name: 'World History World Spine' })).toBeTruthy(); expect(rail.getAttribute('aria-hidden')).toBe('false'); fireEvent.keyDown(document, { key: 'Escape' }); expect(rail.getAttribute('aria-hidden')).toBe('true'); expect(open).toBe(document.activeElement); });
  it('navigates to a semantic section and moves focus with the journey control', async () => { const gateway = new TestGateway(); render(<LearnApp lessonId="lesson.uruk.first-city" gatewayFactory={async () => gateway} />); await screen.findByRole('heading', { name: 'Uruk: Life in an Early City' }); await userEvent.click(screen.getByRole('button', { name: 'Open World Spine' })); await userEvent.click(screen.getByRole('button', { name: 'The built city' })); const section = document.getElementById('section.uruk.the-built-city'); expect(section?.scrollIntoView).toHaveBeenCalledWith({ behavior: 'smooth', block: 'start' }); expect(section).toBe(document.activeElement); });
  ...
  it('keeps the Chronos shell and prerequisite visible for a locked lesson', async () => {
    ...
    expect(document.querySelectorAll('.global-rail nav a')).toHaveLength(3);
    const rail = screen.getByLabelText('World History World Spine');
