...
describe('Learn route and interactions', () => {
  ...
  it('opens the published Farming and Settlements lesson', async () => {
    ...
    const map = screen.getByRole('img', { name: /Editorial locator map.*Çatalhöyük/i });
    ...
    expect(screen.queryByText('Site coordinate verified · modern coasts for orientation')).toBeNull();
  ...
  it('gives the portrait Human Origins map the same intentional pair without trapping its prose in the grid', async () => {
    ...
    const map = screen.getByRole('img', { name: /Map of Africa with five marked sites/i });
    ...
    expect(map.closest('figure')?.querySelector('figcaption')).toBeNull();
    expect(screen.queryByText('Coordinates verified per site · modern coasts for orientation')).toBeNull();
  });
  ...
  it('records a sincere concise response before the learner has to leave the field', async () => { const gateway = new TestGateway(); render(<LearnApp lessonId="lesson.uruk.first-city" gatewayFactory={async () => gateway} />); await screen.findByRole('heading', { name: 'Uruk: Life in an Early City' }); await userEvent.click(screen.getByLabelText('Administrative tablets and cylinder seals')); const text = screen.getByRole('textbox'); await userEvent.type(text, 'Specialized work created opportunity, while unequal labor was a serious cost.'); await waitFor(() => expect(screen.getByRole('button', { name: 'Complete lesson' }).hasAttribute('disabled')).toBe(false)); expect(text).toBe(document.activeElement); expect(gateway.saveAttempt).toHaveBeenCalledWith('lesson.uruk.first-city', 'prompt.uruk.opportunity-and-cost', expect.stringMatching(/Specialized work/)); });
