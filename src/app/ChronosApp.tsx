...
import { ArrowRight, Bookmark, Check, ChevronRight, Clock3, Compass, Home, Library, Moon, Search, Sun } from 'lucide-react';
...
function GlobalNav({ active, theme, onTheme }: { active: 'home' | 'library' | 'search'; theme: 'light' | 'dark'; onTheme(): void }) {
  const items = [
  ...
  return <>
    <aside className="global-rail" aria-label="Chronos navigation">
      <a className="app-brand" href="/home" aria-label="Chronos Home"><span aria-hidden="true">C</span><strong>Chronos</strong></a>
      <nav>{items.map(({ id, label, href, icon: Icon }) => <a key={id} href={href} className={active === id ? 'active' : ''} aria-current={active === id ? 'page' : undefined}><Icon /><span>{label}</span></a>)}</nav>
      <button className="theme-button" onClick={onTheme} aria-label={`Use ${theme === 'light' ? 'dark' : 'light'} theme`}>{theme === 'light' ? <Moon /> : <Sun />}</button>
    </aside>
    <nav className="mobile-nav" aria-label="Chronos navigation">{items.map(({ id, label, href, icon: Icon }) => <a key={id} href={href} className={active === id ? 'active' : ''} aria-current={active === id ? 'page' : undefined}><Icon /><span>{label}</span></a>)}</nav>
  </>;
}
...
function HomePage({ content, snapshot, summaries, onCommit }: { content: ChronosContentBundle; snapshot: JourneyStateLoad; summaries: Record<string, JourneyProgressSummary>; onCommit(next: LearnerJourneyState, destination?: string): void }) {
  ...
  const activeLesson = nextAction.kind === 'lesson' ? content.lessons.find((lesson) => lesson.id === nextAction.lessonId && lesson.status === 'published') : undefined;
...
function LibraryPage({ content, state, summaries }: { content: ChronosContentBundle; state: LearnerJourneyState; summaries: Record<string, JourneyProgressSummary> }) {
  ...
  return <main className="discovery-main library-page"><header className="page-intro library-intro"><div><p className="label">Authored journeys</p><h1>Explore history.</h1><p>Choose a reviewed path with a clear question and intentional sequence.</p></div><a className="search-entry" href="/search"><Search /> Search published history <kbd>Ctrl K</kbd></a></header>
    ...
    <div className="category-stack">{(Object.keys(categoryCopy) as LibraryCategory[]).map((category) => { const copy = categoryCopy[category]; const items = catalog.groups[category]; return <section key={category} className="library-category" aria-labelledby={`category-${category}`}><div className="section-title"><div><p className="label">{category === 'investigations' ? 'Evidence and uncertainty' : 'Optional journeys'}</p><h2 id={`category-${category}`}>{copy.title}</h2><p>{copy.description}</p></div></div>{items.length ? <div className="journey-grid">{items.map((item) => <React.Fragment key={item.id}><JourneyCard item={item} state={state.journeys[item.id]} progress={deriveJourneyProgress(content.journeys.find((journey) => journey.id === item.id)!, content.lessons, summaries)} /></React.Fragment>)}</div> : <div className="empty-category"><span aria-hidden="true">{category === 'investigations' ? '?' : '—'}</span><div><strong>Curated journeys are being prepared.</strong><p>Nothing appears here until the complete journey and its entry lessons are published.</p></div></div>}</section>; })}</div>
  </main>;
}
...
function JourneyDetailPage({ journeyId, content, state, summaries, busy, onCommit }: { journeyId: string; content: ChronosContentBundle; state: LearnerJourneyState; summaries: Record<string, JourneyProgressSummary>; busy: boolean; onCommit(next: LearnerJourneyState, destination?: string): void }) {
  ...
  return <main className="discovery-main detail-page"><a className="back-link" href="/library">← Back to Library</a><header className="detail-hero"><p className="label">{kindLabel(journey.kind)}</p><h1>{journey.title}</h1><p className="detail-question">{journey.openingQuestion}</p><p>{journey.description}</p><div className="feature-meta"><span>{journey.period}</span>{journey.region && <span>{journey.region}</span>}<span>About {journey.approximateMinutes} minutes</span><span>{item.lessonCount} published lessons</span></div><ProgressLine progress={progress} /><div className="detail-actions">{nextAction.kind === 'lesson' ? <a className="primary-action" href={`/learn/${nextAction.lessonId}`} onClick={(event) => { event.preventDefault(); onCommit(continueState, `/learn/${nextAction.lessonId}`); }}>{record?.status === 'open' ? 'Continue journey' : 'Start journey'} <ArrowRight /></a> : <p className="journey-next-state" role="status">{nextAction.kind === 'complete' ? 'All published required lessons are complete.' : 'The next required lesson is waiting on an earlier curriculum prerequisite.'}</p>}{journey.id !== 'journey.world-history' && record?.status === 'open' && <button disabled={busy} onClick={() => onCommit(closeJourney(state, journey.id))}>Close</button>}{record?.status === 'saved' ? <button disabled={busy} onClick={() => onCommit(openJourney(state, journey, content.lessons))}>Open journey</button> : journey.id !== 'journey.world-history' && record?.status !== 'open' && <button disabled={busy} onClick={() => onCommit(saveJourney(state, journey, content.lessons))}>Save for later</button>}</div></header>
    <section className="detail-layout"><div><p className="label">What this journey promises</p><h2>{journey.learnerPromise}</h2><p>{journey.kind === 'world-history' ? 'Required entries form the chronological core. Only reviewed, published lessons count toward the progress shown here.' : 'Required lessons define the authored path; optional entries add depth without blocking completion.'}</p></div><dl><div><dt>Scope</dt><dd>{journey.kind === 'world-history' ? 'Core chronological journey' : 'Optional authored journey'}</dd></div><div><dt>Prerequisites</dt><dd>{journey.prerequisiteJourneyIds.length ? `${journey.prerequisiteJourneyIds.length} curated prerequisite` : 'None'}</dd></div><div><dt>Status</dt><dd>{progress.isCompleted ? 'Completed' : record?.status === 'saved' ? 'Saved' : record?.status === 'open' ? 'Open' : 'Available'}</dd></div></dl></section>
...
export function DiscoveryApp({ route, content = chronosContent, journeyGatewayFactory = createJourneyStateGateway, progressGatewayFactory = createProgressGateway, searchProvider, navigate = (destination) => window.location.assign(destination) }: DiscoveryAppProps) {
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() => { const stored = localStorage.getItem('chronos.theme.v1'); return stored === 'dark' || stored === 'light' ? stored : matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'; });
  ...
  useEffect(() => { let current = true; setLoadError(''); Promise.all([journeyGatewayFactory(), progressGatewayFactory()]).then(async ([journeyGateway, progressGateway]) => { gatewayRef.current = journeyGateway; const publishedIds = content.lessons.filter((lesson) => lesson.status === 'published').map((lesson) => lesson.id); const [loaded, progress] = await Promise.all([journeyGateway.load(), progressGateway.loadJourneySummaries(publishedIds)]); if (current) { setSnapshot(loaded); setSummaries(progress); } }).catch(() => { if (current) setLoadError('Your navigation state could not be loaded. Check the connection and retry.'); }); return () => { current = false; }; }, [attempt, content, journeyGatewayFactory, progressGatewayFactory]);
  ...
  const setAppTheme = () => { const next = theme === 'light' ? 'dark' : 'light'; localStorage.setItem('chronos.theme.v1', next); setTheme(next); };
  if (loadError) return <div className="discovery-app" data-theme={theme}><Recovery title="We couldn’t open your journeys." body={loadError} retry={() => { setSnapshot(undefined); setAttempt((value) => value + 1); }} /></div>;
  if (!snapshot) return <main className="discovery-loading" aria-busy="true">Opening your Chronos library…</main>;
  const active = route.name === 'home' ? 'home' : route.name === 'library' || route.name === 'journey' ? 'library' : 'search';
  return <div className="discovery-app" data-theme={theme}><GlobalNav active={active} theme={theme} onTheme={setAppTheme} />{actionError && <div className="action-error" role="alert">{actionError}</div>}{route.name === 'home' && <HomePage content={content} snapshot={snapshot} summaries={summaries} onCommit={commit} />}{route.name === 'library' && <LibraryPage content={content} state={snapshot.state} summaries={summaries} />}{route.name === 'journey' && <JourneyDetailPage journeyId={route.journeyId} content={content} state={snapshot.state} summaries={summaries} busy={busy} onCommit={commit} />}{route.name === 'search' && <SearchPage provider={provider} initialQuery={route.query} />}{route.name === 'not-found' && <Recovery title="That page isn’t in the archive." body="Check the address or return to a published Chronos destination." />}</div>;
}
