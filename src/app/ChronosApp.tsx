...
function ProgressLine({ progress }: { progress: ReturnType<typeof deriveJourneyProgress> }) {
  return <div className="progress-line" aria-label={`${progress.completed} of ${progress.total} lessons completed`}><span style={{ width: `${progress.percent}%` }} /><small>{progress.completed} of {progress.total} lessons</small></div>;
}
...
function HomePage({ content, snapshot, summaries, inbox, onCommit }: { content: ChronosContentBundle; snapshot: JourneyStateLoad; summaries: Record<string, JourneyProgressSummary>; inbox?: ReviewInbox; onCommit(next: LearnerJourneyState, destination?: string): void }) {
  ...
  const chapter = activeJourney?.chapters.find((candidate) => candidate.entries.some((entry) => entry.lessonId === activeLesson?.id));
  const connection = activeLesson ? learningConnectionsByLessonId[activeLesson.id] : undefined;
  const retrieveLesson = connection?.retrieve && summaries[connection.retrieve.lessonId]?.status === 'completed' ? content.lessons.find((lesson) => lesson.id === connection.retrieve!.lessonId && lesson.status === 'published') : undefined;
  ...
  return <main className="discovery-main home-page"><header className="page-intro"><p className="label">Your learning</p><h1>Welcome back.</h1><p>One clear next step, with room to explore when you choose.</p></header>
    ...
    <section className="dashboard-section discovered" aria-labelledby="discovered-title"><div className="section-title"><div><p className="label">Memory anchors</p><h2 id="discovered-title">Recently discovered</h2></div></div>{snapshot.ownedCards.length ? <div className="discovered-grid">{snapshot.ownedCards.slice(0, 4).map(({ cardId }) => { const card = cardById.get(cardId); const media = card ? mediaById.get(card.mediaId) : undefined; return card ? <a key={card.id} className="mini-card" href={`/learn/${card.unlockLessonId}`}>{media && <ResponsiveMedia media={media} alt="" sizes="180px" loading="lazy" />}<span>{knowledgeCardTypeLabel(card.category)}</span><strong>{card.title}</strong></a> : null; })}</div> : <div className="limited-state"><Bookmark /><div><strong>Your discoveries will gather here.</strong><p>When a parent passes one of your lessons, its Knowledge Card gathers here to keep its big idea close.</p></div></div>}</section>
  </main>;
}

function LibraryPage({ content, state, summaries, onOpenWorldHistory }: { content: ChronosContentBundle; state: LearnerJourneyState; summaries: Record<string, JourneyProgressSummary>; onOpenWorldHistory(event: React.MouseEvent<HTMLButtonElement>): void }) {
  const catalog = createPublishedJourneyCatalog(content.journeys, content.lessons);
  const categories = Object.keys(categoryCopy) as LibraryCategory[];
  const lessons = content.lessons.filter((lesson) => lesson.status === 'published');
  const world = content.journeys.find((journey) => journey.id === catalog.worldHistory?.id);
  const orderedIds = world?.chapters.flatMap((chapter) => chapter.entries.map((entry) => entry.lessonId)) ?? [];
  const subjects = [...lessons].sort((a, b) => orderedIds.indexOf(a.id) - orderedIds.indexOf(b.id));
  useEffect(() => { document.title = 'Library · Chronos'; }, []);
  return <main className="discovery-main library-page">
    <header className="page-intro"><p className="label">The history library</p><h1>Explore history.</h1><p>Meet early humans, examine surviving objects, and ask how people changed the ways they lived.</p></header>
    {catalog.worldHistory && world && <section className="world-feature" aria-labelledby="world-feature-title"><div><p className="label">Start here · Your chronological journey</p><h2 id="world-feature-title">{catalog.worldHistory.title}</h2><p>{catalog.worldHistory.learnerPromise}</p><div className="feature-meta"><span>{catalog.worldHistory.period}</span><span>{catalog.worldHistory.lessonCount} lessons available</span></div><ProgressLine progress={deriveJourneyProgress(world, content.lessons, summaries)} /><button className="primary-action" onClick={onOpenWorldHistory}>Open World History <ArrowRight /></button></div><Compass aria-hidden="true" /></section>}
    <section className="dashboard-section" aria-labelledby="subjects-title"><div className="section-title"><div><p className="label">Available to explore</p><h2 id="subjects-title">People, places, and evidence</h2></div><a href="/search">Find a subject <Search /></a></div><div className="subject-grid">{subjects.map((lesson) => {
      const media = content.media.find((item) => item.id === lesson.heroMediaId);
      const evidence = lesson.sections.flatMap((section) => section.modules).find((module) => module.type === 'evidence');
      return <article className="subject-card" key={lesson.id}>{media && <ResponsiveMedia media={media} alt="" sizes="(max-width: 620px) 100vw, (max-width: 1100px) 45vw, 30vw" loading="lazy" />}<div><p className="label">{lesson.masthead} · {lesson.place}</p><h3><a href={`/learn/${lesson.id}`}>{lesson.title}</a></h3><p>{lesson.significance}</p>{evidence?.type === 'evidence' && <p className="subject-object">Examine: {evidence.artifactLabel}</p>}</div></article>;
    })}</div></section>
...
function resolveWorldHistoryDrawer(
  content: ChronosContentBundle,
  state: LearnerJourneyState | undefined,
  summaries: Record<string, JourneyProgressSummary>,
) {
  const journey = content.journeys.find(
    (candidate) => candidate.id === 'journey.world-history' && candidate.status === 'published',
  );
  if (!journey) return undefined;
  const record = state?.journeys[journey.id];
  const nextAction = state
    ? selectJourneyNextAction(journey, content.lessons, summaries, record?.activeLessonId)
    : undefined;
  const candidateIds = [
    nextAction?.kind === 'lesson' ? nextAction.lessonId : undefined,
    record?.activeLessonId,
    journey.entryLessonId,
  ];
  const lesson = candidateIds
    .map((id) => content.lessons.find((candidate) => candidate.id === id && isLessonOpenable(candidate)))
    .find(Boolean)
    ?? content.lessons.find((candidate) => isLessonOpenable(candidate));
  if (!lesson) return undefined;
  return {
    journey,
    lesson,
    state: createJourneyDrawerState(lesson.id, summaries[lesson.id]),
  };
}
...
export function DiscoveryApp({ route, content = chronosContent, journeyGatewayFactory = createJourneyStateGateway, progressGatewayFactory = createProgressGateway, searchProvider, navigate = (destination) => window.location.assign(destination) }: DiscoveryAppProps) {
  ...
  const navigation = <GlobalNavigation
    active={route.name === 'home' ? 'home' : route.name === 'library' || route.name === 'journey' ? 'library' : route.name === 'search' ? 'search' : undefined}
    theme={theme}
    onTheme={toggleTheme}
  ...
  const worldHistoryDrawer = drawerContext ? <JourneyDrawer
    ...
    onNavigate={() => navigate(`/learn/${drawerContext.lesson.id}`)}
    lesson={drawerContext.lesson}
    journey={drawerContext.journey}
    lessons={content.lessons}
    summaries={summaries}
    currentState={drawerContext.state}
    currentSectionId={drawerContext.lesson.sections[0]?.id}
