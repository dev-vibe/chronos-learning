...
function JourneyRail({ open, onClose, onNavigate, explored, currentSectionId, returnFocus }: { open: boolean; onClose(): void; onNavigate(id: string): void; explored: string[]; currentSectionId?: string; returnFocus: React.RefObject<HTMLButtonElement | null> }) {
  ...
  return <><div className={`learn-scrim ${open ? 'is-open' : ''}`} onClick={close} /><aside ref={railRef} className={`journey-rail ${open ? 'is-open' : ''}`} aria-label="World History journey" aria-modal={open || undefined} role={open ? 'dialog' : undefined}>
    ...
    <nav className="section-nav" aria-label="Lesson sections">{lesson.sections.map((section) => <button key={section.id} className={currentSectionId === section.id ? 'active' : ''} aria-current={currentSectionId === section.id ? 'location' : undefined} onClick={() => { onNavigate(section.id); onClose(); }}><span className={explored.includes(section.id) ? 'done' : ''} />{section.heading}</button>)}</nav>
  </aside></>;
}
