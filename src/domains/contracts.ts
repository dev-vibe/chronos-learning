...
export const LessonModuleSchema = z.discriminatedUnion('type', [
  ...
  BaseModule.extend({ type: z.literal('evidence'), title: z.string().min(1), artifactLabel: z.string().min(1), body: z.string().min(1), mediaId: StableId, layout: z.enum(['side-by-side', 'stacked']).optional(), scaleNote: z.string().min(1).optional() }),
