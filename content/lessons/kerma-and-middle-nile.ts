...
import { kermaMedia, kermaMediaSources, kermaVisualModules } from './kerma-media';
...
export const kermaClaims: Claim[] = [
  { id: 'claim.kerma.deffufa', statement: 'The Western Deffufa is a substantial surviving mud-brick monument in the ancient city of Kerma, interpreted as a temple. The site has received conservation and restoration; its present appearance is not the complete original building.', kind: 'interpretation', certainty: 'high', sourceIds: ['source.kerma.map-site', 'source.kerma.deffufa-photo'], reviewStatus: 'reviewed' },
  { id: 'claim.kerma.met-beaker', statement: 'Met 20.2.45 is a handmade Classic Kerma pottery beaker dated ca. 1802–1640 BCE, 10.8 cm tall, found at Abydos Tomb 525 in Egypt. Its flaring form has a dark upper body and interior, red lower body and irregular pale band. Its maker and exact workshop are unidentified.', kind: 'observation', certainty: 'high', sourceIds: ['source.kerma.met-beaker'], reviewStatus: 'reviewed' },
  { id: 'claim.kerma.locator-geography', statement: 'Modern geographic data locate Kerma in northeastern Africa beside the Nile in Sudan, south of Egypt. The UNESCO tentative-list point represents the site; the Africa inset outlines the main map view rather than political territory. Modern river courses and terrain are orientation, not a reconstruction of ancient channels or vegetation.', kind: 'observation', certainty: 'high', sourceIds: ['source.kerma.map-site', 'source.kerma.map-relief', 'source.kerma.map-river', 'source.kerma.isotopes-2008'], reviewStatus: 'reviewed' },
...
const sections: Lesson['sections'] = [
  ...
  },
  {
    ...
    modules: [
      { id: 'module.kerma.beaker', type: 'prose', claimIds: ['claim.kerma.met-beaker'], sourceIds: ['source.kerma.met-beaker'], body: 'Craftspeople are people skilled at making things. One surviving beaker gives a Kerma-period maker’s work a recognizable form. Pottery is clay hardened by heat; this vessel was shaped by hand.\n\nMade around 1800–1640 BCE, it is about eleven centimetres tall and widens toward the opening. Look at the photograph: its dark upper surface and red lower body meet an irregular pale band. The vessel combines a carefully made shape with a distinctive finish.\n\nThis Classic Kerma-style beaker was found at Abydos in Egypt, not in Kerma city. It is now in the Metropolitan Museum of Art, numbered 20.2.45. Its maker and exact workshop are unknown. We can recognize skilled work without claiming to know exactly who made it or how it reached Egypt.' },
      kermaVisualModules.beaker,
...
export const kermaLesson: Lesson = {
  ...
  orientationMapModuleId: 'module.kerma.nile-locator',
  sectionIdsRequired: sections.map((section) => section.id), sections,
  claimIds: kermaClaims.map((claim) => claim.id), sourceIds: kermaSources.map((source) => source.id), mediaIds: kermaMedia.map((media) => media.id), promptIds: kermaPrompts.map((prompt) => prompt.id),
};

export const kermaContent: AuthoredContentModule = { sources: kermaSources, claims: kermaClaims, media: kermaMedia, lessons: [kermaLesson], prompts: kermaPrompts };
