...
const sections: Lesson['sections'] = [
  ...
  },
  {
    ...
    modules: [
      { id: 'module.akkad.stele', type: 'prose', claimIds: ['claim.akkad.stele-image', 'claim.akkad.stele-afterlife'], sourceIds: ['source.akkad.louvre-stele'], body: 'Naram-Sin’s victory stele is a tall carved stone made to commemorate a victory. It shows the ruler above his soldiers, much larger than the defeated people. His size and height in the composition make his power the center of the scene. The figures of defeated people make the violence of conquest visible without telling us their names or words.\n\nThe arrangement asks a viewer to see the ruler as powerful. That is evidence of a message, not a complete report of how the fighting happened or how anyone governed afterward. The stone has another history, too: it is associated with Sippar and was later taken to Susa, where another ruler added an inscription about its removal. Much of Naram-Sin’s own writing on it is lost. The artwork survives, but its two moments and its missing parts matter.\n\nThe stele presents Akkadian power through art. The buildings and sealings from Leilan and Urkesh answer a different question: what traces of continuing relationships did rulers leave at particular places?' },
    ],
  },
  {
...
export const akkadLesson: Lesson = {
  ...
  claimIds: akkadClaims.map((claim) => claim.id), sourceIds: akkadSources.map((source) => source.id), mediaIds: [], promptIds: akkadPrompts.map((prompt) => prompt.id),
};

export const akkadContent: AuthoredContentModule = { sources: akkadSources, claims: akkadClaims, lessons: [akkadLesson], prompts: akkadPrompts };
