...
select is(
  (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname in ('submit_lesson','review_submission','link_learner','learner_link_code','acknowledge_pass',
     'add_learner_profile','remove_learner_profile','parent_pin_enabled','set_parent_pin','verify_parent_pin','can_act_as')
     and not p.prosecdef),
  0::bigint,
  'every command runs as a checked security definer'
);
select table_privs_are('public', 'parent_view_pins', 'authenticated', array[]::text[], 'PIN hashes are never readable from the browser');
select function_privs_are('public', 'add_learner_profile', array['text'], 'anon', array[]::text[], 'anonymous users cannot add profiles');
select function_privs_are('public', 'set_parent_pin', array['text'], 'anon', array[]::text[], 'anonymous users cannot set a PIN');
select col_is_fk('public', 'learners', 'account_id', 'every learner belongs to a sign-in');
