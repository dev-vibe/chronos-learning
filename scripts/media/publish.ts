...
type ManifestAsset = { id: string; source: StoredObject; locator: { bucket: string; variants: Array<Omit<StoredObject, 'bucket'>> } };
...
const selectedMedia = requestedIds.length === 0 ? media : media.filter((asset) => requestedIds.includes(asset.id));
const missingIds = requestedIds.filter((id) => !selectedMedia.some((asset) => asset.id === id));
if (missingIds.length > 0) throw new Error('Unknown media asset IDs: ' + missingIds.join(', '));
if (!sourcesOnly && !verifyOnly) {
  const blockedAssets = selectedMedia.filter((asset) => asset.reviewStatus !== 'approved');
...
for (const object of objects) {
  ...
  if (existing.error && String((existing.error as { statusCode?: string }).statusCode) !== '404') throw existing.error;
  ...
  const uploaded = await storage.upload(object.objectKey, bytes, {
