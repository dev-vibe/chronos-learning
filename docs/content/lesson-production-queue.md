...
| 90 | `lesson.nubia.kerma-and-nile-world` — Kerma and the Middle Nile | 17 | Explain the Nile as a connected political world rather than an Egypt-only story. | Publication authorized September 17; migration applied and 11 media objects remotely verified; hosted smoke blocked on Vercel sign-in, then merge and production verification pending | `Review` | [ASH-101](https://linear.app/ashs-workshop/issue/ASH-101/research-and-publish-kerma-and-the-middle-nile) · [PR #47](https://github.com/dev-vibe/chronos-learning/pull/47) · [Research record](../research/kerma-and-middle-nile.md) |
...
The final branch deployment and PR checks are green, but the delegated hosted smoke reached Vercel authentication on both direct and audit links before lesson content loaded. Carlin has the exact preview and completion/reopen checklist. Keep the row in `Review` and do not merge or start Akkad until that smoke result is returned.
