...
Selection on August 19, 2026 returned to the lowest eligible `Ready` row: Caral and Early Andean Urbanism under ASH-98. Carlin approved the Stages 0–14B prototype on August 19. Stage 15 implementation, media, migration, and the implementation gate are complete; the queue row is now `Review`. It is also the only reviewed future candidate remaining, so the curriculum queue needs replenishment before the following production cycle; Planned rows still require explicit promotion.
