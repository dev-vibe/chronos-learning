...
| 15 | `lesson.humans.sahul-crossing` — Crossing to Sahul | 3 | Product-owner direction (2026-09-03): backfill first-era World Spine position 3 in parallel with Pyramids, Builders, and Evidence. | `lesson.humans.migrations-and-interbreeding` merged and verified; publication and media verification recorded September 4; PR merged and ASH-109 Done | `Complete` | [ASH-109](https://linear.app/ashs-workshop/issue/ASH-109/research-and-publish-crossing-to-sahul) · [PR #35 merged](https://github.com/dev-vibe/chronos-learning/pull/35) |
...
| 70 | `lesson.egypt.pyramids-and-state-labor` — Pyramids, Builders, and Evidence | 15 | Teach how context, phase, date, attribution, use, and purpose differ, using Giza plus concise Osirion, Hawara, and stone-vessel comparisons. | Product-owner final approval recorded September 3; migration, media checksums, and hosted preview verified; PR merged and production deployment READY; live walkthrough left to Carlin at his request | `Complete` | [ASH-99](https://linear.app/ashs-workshop/issue/ASH-99/research-and-publish-pyramids-builders-and-evidence) · [PR #29 merged](https://github.com/dev-vibe/chronos-learning/pull/29) |
| 80 | `lesson.indus.cities-and-signs` — Indus Cities and Undeciphered Signs | 16 | Add a major independent urban tradition and teach disciplined inference from archaeology when writing cannot be read. | Early Writing Systems merged and verified; parallel production explicitly authorized by Carlin September 9, 2026 | `Complete` | [ASH-100](https://linear.app/ashs-workshop/issue/ASH-100/research-and-publish-indus-cities-and-undeciphered-signs) · [PR #41](https://github.com/dev-vibe/chronos-learning/pull/41) · Approved and published September 13. PR #41 merged; production verified. [Live lesson](https://chronos-learning.vercel.app/learn/lesson.indus.cities-and-signs). |
| 90 | `lesson.nubia.kerma-and-nile-world` — Kerma and the Middle Nile | 17 | Explain the Nile as a connected political world rather than an Egypt-only story. | Published September 17; migration and 11 media objects verified; authenticated hosted smoke passed; PR merged and production deployment verified | `Complete` | [ASH-101](https://linear.app/ashs-workshop/issue/ASH-101/research-and-publish-kerma-and-the-middle-nile) · [PR #47 merged](https://github.com/dev-vibe/chronos-learning/pull/47) · [Live lesson](https://chronos-learning.vercel.app/learn/lesson.nubia.kerma-and-nile-world) · [Research record](../research/kerma-and-middle-nile.md) |
| 100 | `lesson.mesopotamia.akkadian-empire` — Akkad and the Problem of Empire | 18 | Introduce empire as a recurring political strategy rather than a celebratory first. | Early Writing Systems merged and verified | `Complete` | [ASH-102](https://linear.app/ashs-workshop/issue/ASH-102/research-and-publish-akkad-and-the-problem-of-empire) · [Research and publication record](../research/akkad-and-the-problem-of-empire.md) · Carlin approved the hosted lesson check; [PR #49](https://github.com/dev-vibe/chronos-learning/pull/49) merged, and the matching production deployment is READY. The migration is recorded and all nine media objects verify by checksum. |
...
`lesson.egypt.pyramids-and-state-labor` completed publication on September 3, 2026. Carlin approved the final lesson and publication; the committed migration and all 12 selected immutable media objects were verified; the hosted preview completed successfully; PR #29 merged as `cb8a147`; and Vercel production deployment `dpl_Cr54WMzWZKNnBMWmQTBiy73kLkis` reached `READY` at that commit. Carlin explicitly requested to do the live-site check himself; no agent production walkthrough is claimed. [Live lesson](https://chronos-learning.vercel.app/learn/lesson.egypt.pyramids-and-state-labor) · [Approved branch preview](https://chronos-learning-git-codex-ash-99-pyr-4875cc-dev-vibes-projects.vercel.app/audit?on&next=%2Flearn%2Flesson.egypt.pyramids-and-state-labor).

On September 4, 2026, Carlin approved and requested publication of the Hawara image correction. [PR #39](https://github.com/dev-vibe/chronos-learning/pull/39) merged as `4eb1a7d`; production deployment `dpl_G9WeqEWxLmUpSRARUo5jcNwHbcXU` is `READY`. Kircher's imagination card and a separate referenced scan-illustration card are live, with an explicit scale limitation. Seven new immutable media objects were remotely checksum-verified; content validation, 50 publication tests, Vercel checks, and the hosted completion smoke passed. The existing migration, lesson completion requirements, and `Complete` queue status are retained. [Live lesson](https://chronos-learning.vercel.app/learn/lesson.egypt.pyramids-and-state-labor).

On August 31, 2026, the product owner replenished the runway with World Spine positions 15–24 and directed creation of the corresponding per-lesson issues ASH-99 through ASH-108. Animals, Herding, and Mobility and Wheels, Metals, and Specialized Work remain `Planned` and are not auto-selected. Following Indus publication, Kerma is the active lesson; continue it before selecting new work. Rows 110, 120, 130, 140, 150, and 160 stay `Ready` but ineligible until their named production dependencies are satisfied.

On September 14, 2026, the next-lesson boot reconciled Sahul's stale `Review` label against merged PR #35, ASH-109 `Done`, and the [research record](../research/crossing-to-sahul.md#final-implementation-and-release-work) documenting the applied publication migration and verified media on September 4. This records existing release evidence, not a new hosted walkthrough. Kerma started from current main `308ad73` in `.worktrees/ash-101-kerma`, branch `codex/ash-101-kerma-and-middle-nile`. Its [research record](../research/kerma-and-middle-nile.md) owns the Stage 3B packet, subsequent owner approvals and current implementation record. Existing checkout changes remain untouched.

Later on September 14, Carlin approved Kerma's revised research direction after the audience-fit self-review. The same branch now contains the full six-section unpublished lesson and two working prompts, with planned map/object-photo annotations and a no-card ending. [Local learner prototype](http://localhost:3000/learn/lesson.nubia.kerma-and-nile-world) requires the running isolated-worktree preview server. At that checkpoint the row remained `Researching` and product review was pending; research-direction approval alone did not approve final media. Carlin subsequently inspected the running lesson and said “looks good” and “please continue”, authorizing final media. No separate independent adult-proxy session is claimed. Publication remains a later explicit decision.

On September 16, the source-anchored locator, Met beaker, Western Deffufa and Lady Sennuwy visuals passed provenance, delivery and bounded desktop/mobile rendering checks. Implementation and release gates, content validation, 63 domain/content/lesson tests and Chronos typecheck pass. Vercel built the [hosted review preview](https://chronos-learning-git-codex-ash-101-ke-fede14-dev-vibes-projects.vercel.app/audit?on&next=%2Flearn%2Flesson.nubia.kerma-and-nile-world), which requires authorized preview access. Kerma is now `Review`; it remains a draft with no publication migration, hosted media upload or unlock until explicit authorization.

On September 17, Carlin explicitly authorized publication with “nice. publish!” The canonical cutover set the lesson to published, generated and applied migration `20260917112038_publish_kerma_and_nile_world`, passed all 17 rollback database assertions, and uploaded then checksum-verified all 11 Kerma source/delivery media objects. A generated prototype-registry removal defect and the missing published learning connection were corrected before the prescribed 63 tests passed. The row remains `Review` until hosted smoke, green PR checks, merge and production deployment verification.

The final branch deployment and PR checks were green. The delegated hosted smoke reached Vercel authentication on both direct and audit links before lesson content loaded; Carlin completed the supplied authenticated preview and completion/reopen checklist and reported “smoke is fine” on September 17. PR #47 then merged, the main-branch Vercel deployment completed successfully, and the live lesson returned the production app. Kerma is complete; Akkad is now the first eligible Ready lesson.

On September 23, 2026, current main records Akkad Complete after PR #49 merged and the production deployment was verified. Orders 110 and 120 remain ineligible while Wheels and Animals stay Planned. Law, Kingship, and Hammurabi's Babylon (order 130) is the first eligible row and has begun Stage 0–3B research on `codex/ash-105-law-kingship-babylon` from main `6b2e270` in an isolated worktree. Its [research-direction packet](../research/law-kingship-hammurabi-babylon.md) awaits Carlin's response before claims or learner content are selected. Existing checkout changes are untouched.
...
On September 9, 2026, Carlin explicitly requested the next eligible lesson on a fresh branch from current `main` in parallel with existing work. Indus Cities and Undeciphered Signs (production order 80) started on `codex/ash-100-indus-cities-and-signs`, based on `ad1b50a`, in a separate worktree. Its [research record](../research/indus-cities-and-signs.md) owns the Stages 0–3B checkpoint. Existing lesson closeout states are preserved; no shared runtime or migration changes are part of this research increment.
