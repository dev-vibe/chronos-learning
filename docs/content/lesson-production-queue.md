...
`lesson.farming.multiple-origins` is the single active row. Carlin approved its Stages 0–14B learner prototype on August 11, 2026. Stage 15 implementation and the Stage 16 production-preview gate are complete on draft PR #22. The required reference-tier ages 11–14 walkthrough remains the only lesson-specific release gate and does not authorize publication work to begin early.
