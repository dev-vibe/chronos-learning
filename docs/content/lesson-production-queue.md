...
Selection on August 19, 2026 returned to the lowest eligible `Ready` row: Caral and Early Andean Urbanism under ASH-98. Carlin approved the Stages 0–14B prototype on August 19 and confirmed the lesson was done on August 27. The authored lesson is `published`; the release gate and post-publication suite passed. Hosted Chronos development apply, immutable media upload, merge, and queue `Complete` remain because the hosted project is paused in this session. It is also the only reviewed future candidate remaining, so the curriculum queue needs replenishment before the following production cycle; Planned rows still require explicit promotion.
