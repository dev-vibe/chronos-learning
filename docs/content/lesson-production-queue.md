...
`lesson.egypt.nile-state` completed production on August 19, 2026. Its Stages 0–14B packet and independent adult learner-proxy review found no blockers; Carlin approved the revised prototype, all three visual jobs, the Narmer Palette Artifact / Witness card, and the safe journey-end behavior. The release gate, responsive/product checks, committed migration, hosted configuration, immutable media publication, and post-publication regression suite all passed. Caral is now the first eligible Ready row. It is also the only reviewed future candidate remaining, so the curriculum queue needs replenishment before the following production cycle; Planned rows still require explicit promotion.
...
| `lesson.egypt.nile-state` — The Nile and an Early Egyptian State | 13 | [ASH-97](https://linear.app/ashs-workshop/issue/ASH-97/research-and-publish-the-nile-and-an-early-egyptian-state) · [PR #24](https://github.com/dev-vibe/chronos-learning/pull/24) | Published 2026-08-19 after the proxy/product gates, three-asset visual lifecycle, hosted migration, and full production validation passed. Decision packet in `docs/research/the-nile-and-an-early-egyptian-state.md`. |
