...
Opening orientation uses reviewed geographic data and existing chronology. A locator must place the subject relative to recognizable wider geography before asking learners to compare local sites: world/continent → recognizable region → lesson places. Supply a wider extent or linked inset and a visible native orientation sentence. Unknown local names alone are insufficient; the relationship must be readable at embedded size without opening a viewer. Maps follow the lively, legible historical-atlas treatment of the Akkad locator, adapted to the subject, through faithful rendering or editing of real, rights-cleared geographic sources and reviewed data. A compact timeline relates the subject to earlier learning and meaningful overlap. Deep time, approximate dates, uncertain locations and any schematic scale are labeled honestly; do not invent precision or expose the raw graph.
...
The [lesson creation runbook](../content/lesson-creation-runbook.md) owns the operational path: **brief → research/material decisions → learning prototype → review → final assets/verification → publication**. The [production queue](../content/lesson-production-queue.md) supplies ordered work and pauses; a next-lesson request invokes their boot sequence. The [Stage 14B review questions](../content/lesson-creation-runbook.md#stage-14b--check-the-prototype-and-request-owner-approval) and [authoring templates](../content/lesson-production/authoring-templates.md) are review and record instruments, not parallel procedures.
...
After explicit approval to publish, use only the [runbook Stage 18](../content/lesson-creation-runbook.md#stage-18--publish-then-correct-when-needed). Preserve authoritative claim/source records and automate mechanical integrity checks; do not repeat completed research or quality reviews. Historical maps use the [map guidance](../content/historical-map-production.md), with reviewed geography and honest uncertainty regardless of rendering method.
...
Follow the runbook's [media method and fidelity policy](../content/lesson-creation-runbook.md#media-method-and-fidelity-policy). Prefer licensed originals for direct evidence. Deterministic/native/vector or raster methods are appropriate for factual diagrams, timelines and comparisons when they reliably preserve reviewed geometry, measurements, labels and relationships. Lesson maps use the owner-selected Akkad visual benchmark in the [map runbook](../content/historical-map-production.md): warm, energetic color, subtle source-faithful terrain, and high-contrast rivers, markers and labels. Choose deterministic rendering or a faithful illustrated edit of reviewed geography according to fidelity and lesson-size legibility; the style reference never supplies geographic facts. Generation is also appropriate for suitable reconstruction and atmospheric art; other diagrams and evidence originals do not require stylistic transformation. Every method retains provenance, rights, uncertainty, accessible equivalents and reference/data-versus-final fidelity review.
