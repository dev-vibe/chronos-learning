...
- Opening orientation uses existing date ranges and available authored map assets. The timeline distinguishes overlap and approximation. The chosen map appears once at the opening, with its sources and qualifications in the enlargement viewer; stable section anchors and teaching text remain. Lessons lacking suitable map assets retain honest place text rather than invented geography.
...
| Uruk (`lesson.uruk.first-city`) | Authored southern Mesopotamia map | Representative integration: opening map, clay-record attention cue and a single-object evidence panel, question evidence, optional card recall/connection, and public companion. Existing source-review qualifications remain visible. |
...
- `npm test`: 183 tests in 34 files pass, covering domain/content, lesson gates, draft preservation, deliberate submission, errors, identity separation, completion/card idempotency, prerequisite access, public companions, and discovery.
...
- Follow-up browser review verifies landscape images above captions, square/portrait images alongside captions, no fixed-height image filler, mobile stacking, keyboard focus return, retained map sources, and no repeated opening maps across all ten published lessons.
