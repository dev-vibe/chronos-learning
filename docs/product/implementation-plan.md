...
- Maps must connect recognizable wider geography to lesson places at embedded size, with an inset or wider extent and a visible native orientation sentence. Follow the Nile map's illustrated subtle-terrain style through edits of real geographic source images; preserve source geometry. Catalogue retrofits need an explicit bounded scope, not an automatic claim that existing maps comply.
